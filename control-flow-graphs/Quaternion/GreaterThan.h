...
#ifndef SYSYCOMPILER_GREATERTHAN_H
#define SYSYCOMPILER_GREATERTHAN_H
class GreaterThan : public Binary {
public:
    GreaterThan(string *assigned, string *integer1, string *integer2) :
    Binary(IRKind::GT_BIN, assigned, integer1, integer2) {}

    void print(ofstream *out) override {
        *out << "greatThan " << assigned << " " << integer1 << " " << integer2 << endl;
    }
    ...
    void globalValueNumbering(map<string, int> *constValue, map<string, string> *copyValue,
        ...
        if ((*previous)[IRKind::GT_BIN]->count(hash)) {
            (*copyValue)[assigned] = (*(*previous)[IRKind::GT_BIN])[hash];
        } else {
            (*(*previous)[IRKind::GT_BIN])[hash] = assigned;
            (*cur)[IRKind::GT_BIN]->insert(hash);
        }
    }
    ...
    void assembly(ofstream *out, Function*) override {
        ...
        if (assigned[0] == '$') {
            *out << "sgt " << assigned << ", " << integer1 << ", " << integer2 << endl;
        } else {
            *out << "sgt $27, " << integer1 << ", " << integer2 << endl;
            *out << "sw $27," << assigned.substr(2) << "($sp)" << endl;
        }
    }
};
#endif //SYSYCOMPILER_GREATERTHAN_H
