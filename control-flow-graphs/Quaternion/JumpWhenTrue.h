...
#ifndef SYSYCOMPILER_JUMPWHENTRUE_H
#define SYSYCOMPILER_JUMPWHENTRUE_H
class JumpWhenTrue : public Quaternion {
    BasicBlock *basicBlock;
    string integer;
public:
    JumpWhenTrue(BasicBlock *basicBlock, string *integer) : Quaternion(IRKind::JUMP_TRUE),
    basicBlock(basicBlock), integer(*integer) {}

    void print(ofstream *out) override {
        *out << "jumpWhenTrue " << *basicBlock->getTag() << " " << integer << endl;
    }
    ...
    void assembly(ofstream *out, Function*) override {
        ...
        *out << "bnez " << integer << ", " << *basicBlock->getTag() << endl;
    }
};
#endif //SYSYCOMPILER_JUMPWHENTRUE_H
